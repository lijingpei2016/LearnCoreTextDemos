...
#pragma mark 存在图片信息则需要获取图片具体位置信息
        ...
        //等到该行CTRun信息
        // FIXME: CTLineGetGlyphRuns 获取CTLine中的所有CTRun信息，CTRun是一块样式相同的内容的集合，CTRun包含了 CTRunDelegateCreate创建的CTRunDelegateRef结构体信息（如果存在）
        NSArray * runObjArray = (NSArray *)CTLineGetGlyphRuns(line);
        //遍历该行CTRun信息
        for (id runObj in runObjArray) {
            //判断该CTRun是否设置有代理,若无代理直接进行下次循环
            CTRunRef run = (__bridge CTRunRef)runObj;
            // FIXME: CTRunGetAttributes获取CTRun中属性，然后通过KVC获取到CTRunDelegateRef结构体
            ...
            CGFloat ascent;
            CGFloat desent;
            // FIXME: 使用CTRunGetTypographicBounds获取CTRunDelegate中的宽度、ascent、desent值
            // 获取CTRunDelegate中的宽度并给上升和下降高度赋值，返回值为delegate对应的width宽度值
